...
const Header = () => {
      ...
      <div className="flex flex-col h-screen border-l lg:flex-row lg:h-max lg:border-l-0 border-[#2A2A2A]">
        <ul className="items-center inline-block lg:ml-9 lg:flex">
          {isOpen && (
            <div className="pt-3">
              <svg
                className="w-8 h-8 ml-auto mr-2 cursor-pointer"
                onClick={handleClick}
          ...
          {links.map((link, index) => (
            <li
              key={index}
              onClick={() => isMobile && setIsOpen((prevState) => !prevState)}
              className="my-5 font-bold text-[14px] mx-14 lg:py-6 lg:my-0 md:mx-36 lg:mx-6"
            >
              <NavLink
                className={({ isActive }) =>
                  isActive ? "border-b-[3px] lg:py-6 border-[#2A2A2A]" : null
                }
                to={link.to}
              >
                {link.name}
            ...
            {username ? (
              <div className="flex items-center gap-x-8 justify-between">
                <NavLink
                  ...
                  to={`/user/${userId}`}
                  className={`${({ isActive }) =>
                    isActive ? "text-black" : "text-primary-200"}`}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
  ...
  return (
    <nav className="fixed bg-yellow-400 z-50 top-0 uppercase text-[#2A2A2A] w-full flex items-center justify-between px-5 font-normal lg:px-16">
      <NavLink to="/">
        <span className="text-xl lg:text-4xl font-bold">NITTE</span>
      </NavLink>
      <div
        className={
          isOpen
            ? "inline-block absolute top-0 bg-yellow-400 h-screen right-0"
